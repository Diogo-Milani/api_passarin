import * as pedidoSerivce from '../services/pedidoServices.js'
import Joi from 'joi'
...
export const listarPedidos = async (req,res) => {
    try {
        ...
        const pedidos = await pedidoSerivce.findAll(minValor,maxValor,email,idPedido)
        if (pedidos.length === 0) {
...
export const listarPedidosId = async (req,res) => {
    try {
        ...
        const pedido = await pedidoSerivce.findByIdPedido(pedido)
        if(!pedido) {
...
export const atualizarPedido = async (req,res) => {
    try {
        const {idPedido} = req.params
        const updated = await pedidoSerivce.update(idPedido,req.body)
        if (!updated) {
...
export const adicionarPedido = async (req,res) => {
    try {
        const novoPedido = await pedidoSerivce.create(req.body)
...
export const deletarPedido = async (req,res) => {
    try {
        const {idPedido} = req.params
        const deleted = await pedidoSerivce.remove(idPedido)
        if(!deleted) {
            return res.status(404).json ({error: 'Pedido não encontrado'})
        }
